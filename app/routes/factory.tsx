...
export default function Factory() {
    ...
    useEffect(() => {
        fetch(`${import.meta.env.VITE_API_URL}/api/factory`)
            .then(res => res.json())
    ...
    return (
        <main className="w-full overflow-x-hidden font-sans">
            ...
            {/* ===== HERO BANNER ===== */}
            <section className="relative w-full h-[500px] sm:h-[550px] md:h-[600px] flex flex-col items-center justify-center text-center px-4 sm:px-8 mt-12">
                <img src={heroImg} alt="factory hero" className="absolute inset-0 w-full h-full object-cover object-center" />
                <div className="absolute inset-0 bg-black/55 z-0" />
                <div className="relative z-10 flex flex-col items-center px-4">
                    <div className="flex items-center gap-2 text-white/70 text-xs sm:text-sm mb-3 sm:mb-4 flex-wrap justify-center">
                        <Link to="/" className="hover:text-white transition-colors">Home</Link>
                    ...
                    <p className="text-white/85 text-xs sm:text-sm md:text-base max-w-2xl leading-relaxed">
                        State-of-the-art facilities meeting the highest global ethical and quality standards.
                    </p>
                </div>
            </section>
            ...
            {factories.length > 0 && (
                <section className="w-full py-14 sm:py-20 px-4 sm:px-8 lg:px-16 xl:px-24 bg-gray-50">
                    <div className="max-w-6xl mx-auto">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            {factories.filter(f => f.image).map((factory) => (
                                <div key={factory._id} className="relative w-full h-72 md:h-80 rounded-xl overflow-hidden group">
                                    <img
                                        src={factory.image}
                                        alt={factory.label}
                                        className="w-full h-full object-cover transition-transform duration-300"
            ...
            {factories.map((factory) => (
                factory.inventoryItems && factory.inventoryItems.length > 0 && (
                    <section key={factory._id} className="w-full py-14 sm:py-20 px-4 sm:px-8 lg:px-16 xl:px-24 bg-white">
                        <div className="max-w-5xl mx-auto">
                            <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-gray-900 text-center mb-8 sm:mb-12">
                                Our Machinery Inventory
                            </h2>
                            <div className="mb-3">
                                <h3 className="text-center text-base sm:text-lg font-bold text-gray-800 mb-4 sm:mb-6">
                                    {factory.machineryType} Machinery
                                </h3>
                            </div>
                            <div className="w-full overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
                                <table className="w-full min-w-[500px] text-sm">
                                    <thead>
                                        <tr className="bg-gray-50 border-b border-gray-200">
                                            <th className="text-left px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-600 uppercase w-16">SL No.</th>
                                            <th className="text-left px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-600 uppercase">Machine Name</th>
                                            <th className="text-left px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-600 uppercase w-32">Brand</th>
                                            <th className="text-right px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-gray-600 uppercase w-24">Quantity</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {factory.inventoryItems.map((row, index) => (
